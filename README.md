...
### What We Chose NOT to Build (Deliberate Boundaries)
...
The pipeline consists of four tightly integrated components:
...
### Intent Taxonomy (6 Classes Defined from Real Data)
1. `software_troubleshooting` — iOS updates, app crashes, boot loops, storage alerts.
2. `battery_power` — Rapid drain, unexpected shutdowns, charging issues.
3. `connectivity` — Wi-Fi, Bluetooth, AirPods pairing, cellular dropouts.
4. `account_security` — Apple ID lockouts, two-factor authentication, password resets.
5. `hardware_damage` — Cracked displays, water ingress, speaker/mic physical failure.
6. `billing_store` — App Store unauthorized charges, subscription cancellations, refunds.
...
## 4. LLM-as-a-Judge & Human Alignment (50 Golden Set Examples)
...
*(Mandatory self-critique section)*
...
Banking77 (**10,003 queries, 77 intents**) was used as a secondary dataset to validate intent classification methodology:
...
- **Cross-Domain Classifier**: A TF-IDF classifier trained on mapped Banking77 data achieves **90.7% cross-domain accuracy**, demonstrating that even out-of-domain labeled intent data produces meaningful separability — confirming our taxonomy design.
...
## 9. Golden Set Construction Notes
...
### 4. Run Full Evaluation Pipeline (<2 Minutes)
```bash
...
├── REPORT.md                    # Technical report reference
